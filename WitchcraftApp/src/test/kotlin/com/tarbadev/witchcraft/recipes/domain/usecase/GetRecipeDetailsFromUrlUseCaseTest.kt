...
class GetRecipeDetailsFromUrlUseCaseTest {
    ...
    @BeforeEach
    fun setUp() {
        subject = GetRecipeDetailsFromUrlUseCase(ingredientFromStringUseCase, convertAndAddSameIngredientUseCase)
    ...
    fun execute() {
        ...
        val returnedRecipe = subject.execute(recipe.originUrl)
        assertEquals(recipe, returnedRecipe)
    }

    @Test
    fun execute_getsRecipeName() {
        val recipe = testResources.recipe

        whenever(convertAndAddSameIngredientUseCase.execute(recipe.ingredients))
                .thenReturn(recipe.ingredients)

        assertEquals(recipe.name, subject.execute(recipe.originUrl).name)
    }

    @Test
    fun execute_getsRecipeIngredients() {
        val recipe = testResources.recipe
        val ingredient1 = Ingredient(
            name = "Little Potato Co. Creamer potatoes (I used Dynamic Duo)",
            quantity = 1.5,
        ...
        val ingredients = subject.execute(recipe.originUrl).ingredients
        assertEquals(recipe.ingredients, ingredients)
    }

    @Test
    fun execute_getsRecipeImageUrl() {
        val recipe = testResources.recipe

        val imgUrl = subject.execute(recipe.originUrl).imgUrl
    ...
    @Test
    fun execute_ImportStepsFromParagraphTag() {
        val steps1 = subject.execute("https://www.cookincanuck.com/one-pot-whole-wheat-pasta-recipe-chicken-spinach/").steps
    ...
    @Test
    fun execute_importStepsFromBigParagraph() {
        val steps1 = subject.execute("https://www.cookincanuck.com/baked-tortellini-with-turkey-butternut-squash-chard-recipe/").steps
